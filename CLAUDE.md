...
- `workflow-kit` — Plan → build pipeline (feature-plan, implement-plan; implement-plan orchestrates on Claude Code with swappable phase executors, has a built-in opus escalation rescue, and delegates PR creation to the consuming project's `/create-pr`)
- `dev-toolkit` — Standalone dev utilities (notify-me, pr-review, pr-retro)
